...
import Blockly, { FieldConfig } from 'blockly/core';
...
export class FieldBitmap extends Blockly.Field {
  imgHeight_: number;
  imgWidth_: number;
  editorPixels_: HTMLDivElement[][] | null;
  blockDisplayPixels_: SVGElement[][] | null;
  mouseIsDown_: boolean;
  valToPaintWith_: any;
  boundEvents_: any;
  initialValue_: any;
   ...
   * configure the field.
   */
  constructor(value = undefined, validator = undefined, config: { height?: number, width?: number } & FieldConfig | undefined = undefined) {
   ...
   */
  doClassValidation_(newValue: number[][] | undefined = undefined) {
    if (!newValue) {
      return null;
    }
   ...
   */
  doValueUpdate_(newValue: number[][] | undefined) {
   ...
   */
  addControlButton_(parent: HTMLElement, buttonText: string, onClick: Function) {
  ...
  private resizedArray<T>(array: T[], length: number, valueFactory: () => T): T[] {
    const result = array.slice(0, length);
    while (result.length < length) {
      result.push(valueFactory());
    }
    return result;
  };

  resizeBitmap(newRows: number, newCols: number) {
    const newVal = this.resizedArray((this.value_ as number[][]).map(row => this.resizedArray(row, newCols, () => 0)), newRows, () => new Array<number>(newCols).fill(0));
    this.applyNewValue(newVal);
    this.fieldGroup_!.innerHTML = '';
    this.initView();
  }

  private applyNewValue(newVal: number[][]) {
   ...
   */
  forAllCells_(func: ((r: number, c: number) => void)) {
    for (let r = 0; r < this.imgHeight_; r++) {
      for (let c = 0; c < this.imgWidth_; c++) {
        func(r, c);
      }
   ...
   */
  createElementWithClassname_<T extends keyof HTMLElementTagNameMap>(elementType: T, className: string) {
   ...
   */
  bindEvent_(element: EventTarget, eventName: string, callback: Function) {
